 ...
 */
function handleAADCallback(req, res, next) {
  const { config } = req;
  ...
  return passport.authenticate('aad', handleAuth)(req, res, next);
}

router.get('/auth/aad', (req, res, next) => {
  // If legacy aad is configured authenticate with that mechanism
  // Otherwise use aad implementation based on openid-client
  return passport.authenticate('aad')(req, res, next);
});

router.get('/auth/aad/callback', handleAADCallback);
