...
- `SQLPAD_LDAP_BIND_CREDETIALS` - The password to bind with for the lookup user.
- `SQLPAD_LDAP_SEARCH_FILTER` - LDAP search filter, e.g. (uid={{username}}) or (sAMAccountName={{username}}) in ActiveDirectory.  Use literal {{username}} to have the given username used in the search 
