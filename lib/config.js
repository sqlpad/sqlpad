...
// load values from db.config
// since this is async, the values won't be immediately available
// this is okay though. any config needed immediately 
// will be passed in via env or cli anyways. 
...
db.config.find({}).exec(function (err, dbItems) {
    ...
    dbItems.forEach(function (item) {
        config.set(item.key, item.value, "config db");
    });
});
