...
clients.restapi = function (query, connection, queryResult, callback) {

  var restapiConfig = {
    host: connection.host,
    port: connection.port,
    user: connection.username,
    catalog: connection.restApiCatalog,
    schema: connection.restApiSchema
  }
  var post_data = query;
  var post_options;
  var post_req;
  
  if (post_data === 'TEST_REST_API') {
	  
	console.log('TEST_REST_API');
	
	post_options = {
		  host: restapiConfig.host,
		  port: restapiConfig.port,
		  path: '/your/enpoint/here',
		  method: 'GET',
		  headers: {
  ...
  } else if (post_data === 'SCHEMA_REST_API') {
	
	// queryResult columns :
	// 		- table_type in {Tables|Views}
	//		- table_schema
	//		- table_name
	//		- column_name
	//		- data_type
