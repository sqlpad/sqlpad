...
clients.presto = function (query, connection, queryResult, callback) {
  ...
  // TODO - current driver does not seem to set catalog
  // docs recommend a query like 'SELECT * FROM catalog.schema.table'
  // This works because of fully qualified table name
  const PrestoClient = require('prestodb')
  const prestoClient = new PrestoClient(prestoConfig)
  prestoClient.sendStatement(query)
    .then(result => {
